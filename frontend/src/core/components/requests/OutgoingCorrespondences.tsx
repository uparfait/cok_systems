...
const OutgoingCard: React.FC<{
  ...
  const dateStr = outgoing.created_at
    ? new Date(outgoing.created_at).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })
    : '';
...
const OutgoingCorrespondences: React.FC<{
  onOutgoingClick: (outgoing: OutgoingDoc) => void;
  onNewOutgoing?: () => void;
  onExport?: () => void;
}> = ({ onOutgoingClick, onNewOutgoing, onExport }) => {
  ...
  const updateUrl = useCallback((p: string, f: string, t: string, pg = 1) => {
  ...
  const totalPages = Math.max(1, Math.ceil((outgoings.length || 0) / limit));
  const startIdx = (page - 1) * limit;
  const pageItems = outgoings.slice(startIdx, startIdx + limit);
          ...
          >
            {loading ? (
              ...
              <div className="text-center py-12 text-sm text-gray-400">
                No outgoing correspondences found
              </div>
            ) : (
              <div>
                {pageItems.map((outgoing) => (
                  <OutgoingCard key={outgoing._id} outgoing={outgoing} onClick={() => onOutgoingClick(outgoing)} />
                ))}
              </div>
            )}
          </div>
...
export default OutgoingCorrespondences;
