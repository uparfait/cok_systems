// ServiceHistoryTab.tsx - Completed (non-inhouse) services with pagination & search
...
const ServiceHistoryTab: React.FC<{ departmentScope?: boolean }> = ({ departmentScope = false }) => {
  ...
  const fetchCompletedVisitors = useCallback(async (silent: boolean = false, page: number = currentPage, query: string = searchTerm) => {
    if (!myId || myId === "undefined") { if (!silent) setLoading(false); return; }
    try {
      if (!silent) setLoading(true);
      let response;
      if (query && query.trim()) {
        response = await serviceDeliveryService.search(query, page, resultsPerPage, false);
      } else {
        response = await serviceDeliveryService.getAll(page, resultsPerPage, false);
      }
  ...
  const handleSearchInput = (value: string) => {
    ...
    if (value && value.trim().length >= 1) {
      searchTimeoutRef.current = setTimeout(() => {
        setCurrentPage(1);
        fetchCompletedVisitors(false, 1, value);
      }, 500);
    } else {
      setCurrentPage(1);
      fetchCompletedVisitors(false, 1, "");
    }
  };

  const handleSearch = () => {
    setCurrentPage(1);
    fetchCompletedVisitors(false, 1, searchTerm);
  };

  const handleRefresh = () => {
    fetchCompletedVisitors(false, currentPage, searchTerm);
  };
  ...
  useEffect(() => {
    if (myId && myId !== "undefined") {
      fetchCompletedVisitors(false);
    } else if (!myId || myId === "undefined") {
      setLoading(false);
    }
  }, [fetchCompletedVisitors, currentPage, searchTerm]);
  ...
  return (
    <div className="space-y-4 w-full" style={{ backgroundColor: NEUTRAL_LIGHT }}>
      ...
      <EmployeeVisitorClicked
        ...
        myProviderId={myId}
        onSaved={() => fetchCompletedVisitors(true)}
        hideActions
      />
    </div>
  );
