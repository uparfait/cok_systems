...
const DepartmentManagementTable: React.FC<DepartmentTableProps> = ({
  ...
  const flattenedDepartments = useMemo(() => {
    ...
    departments.forEach(dept => {
      // Add parent department - check if is_unit is false
      if (!dept.is_unit) {
  ...
  const getDepartmentEmployees = (dept: Department) => {
    ...
    if (Array.isArray(dept.employees)) {
      filtered = dept.employees
    } else {
      filtered = employees.filter((emp: any) =>
        emp.department === dept.name ||
        emp.department?.name === dept.name ||
          ...
          <table className="w-full min-w-[1200px]">
            <thead>
              <tr className="bg-gradient-to-r from-gray-50 to-gray-100 border-b border-gray-200">
                ...
                <th className="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider whitespace-nowrap">Description</th>
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="min-w-0">
                            <p className="text-sm font-normal truncate max-w-xs text-gray-900">
                              {dept.name}
                            </p>
                          </div>
                        </div>
                      ...
                      <td className="px-6 py-4">
                        <div>
                          <p className="text-sm font-medium text-gray-900 truncate">
                            {typeof dept.leader === 'object' && dept.leader
                              ? (dept.leader as any).full_name || (dept.leader as any).email
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center gap-1">
                          ...
                          <button
                            onClick={() => onDelete(dept._id || '', dept.name || 'this department')}
                    ...
                    <div className="space-y-3">
                      {paginateArray(selectedDepartment.services, servicesPage, itemsPerPage).map((service: any) => (
                        ...
                        >
                          ...
                          {service.description && (
                            <p className="text-sm text-gray-600 break-words whitespace-pre-wrap">{service.description}</p>
                          )}
                        </div>
                      ))}
                    </div>
