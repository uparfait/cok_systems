                    ...
                    return (
                      <tr key={`${dept._id}-${isSubDept ? "sub" : "parent"}`} className={`hover:bg-[#F7F9FB] transition-colors ${isSubDept ? "bg-[rgba(41,128,185,0.08)]" : ""}`}>
                        <td className="px-4 py-3"><div className={`flex items-center gap-2 ${isSubDept ? "ml-6" : ""}`}>
                          ...
                          <div className="min-w-0"><div className="flex items-center gap-2"><p className={`text-sm font-normal truncate max-w-48 ${isSubDept ? "text-[#2980B9]" : "text-gray-900"}`} title={dept.name}>{dept.name}</p><span className={`text-[10px] px-1.5 py-0.5 font-semibold uppercase tracking-wide shrink-0 ${dept.is_unit || isSubDept ? "bg-[rgba(41,128,185,0.12)] text-[#2980B9]" : "bg-[rgba(5,109,170,0.1)] text-[#056daa]"}`}>{dept.is_unit || isSubDept ? "Unit" : "Department"}</span></div>{dept.description && <p className="text-xs text-gray-500 truncate max-w-48">{dept.description}</p>}</div></div></td>
                        <td className="px-4 py-3"><span className={`text-xs px-2 py-0.5 font-medium ${isSubDept ? "bg-[rgba(41,128,185,0.08)] text-[#2980B9]" : "bg-gray-100 text-gray-700"}`}>{dept.dpt_id || dept.department_id || "—"}</span></td>
