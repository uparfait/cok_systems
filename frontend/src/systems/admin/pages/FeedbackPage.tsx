// // FeedbackPage - Professional Feedback Management Dashboard
// // Features: Comprehensive feedback analytics, department performance, and detailed insights
...
//   const [departmentNameToIdMap, setDepartmentNameToIdMap] = useState<Map<string, string>>(new Map());
//   const [feedbackList, setFeedbackList] = useState<FeedbackItem[]>([]);
//   const [selectedDepartmentForFeedback, setSelectedDepartmentForFeedback] = useState<string>('');
...
//     setLoading(true);
//     try {
//       // Fetch comprehensive feedback analytics
...
//           nameToIdMap.set(dept.department_name, dept.department_id);
//         });
//         setDepartmentNameToIdMap(nameToIdMap);
//       }
//     } catch (error) {
...
//       fetchDepartments();
//     }
//   }, [isAuthenticated, authLoading, fetchFeedbackData, fetchDepartments]);
...
//   // Fetch feedback messages for a department
//   const fetchDepartmentFeedback = useCallback(async (departmentName: string) => {
//     try {
//       // Clear previous feedback data immediately to prevent stale data
//       setFeedbackList([]);
//       setSelectedDepartmentForFeedback(departmentName); // Set immediately to show the section
//       setFeedbackLoading(true);

//       // Get department ID from name
//       const departmentId = departmentNameToIdMap.get(departmentName) || departmentName;

//       // If department ID is the same as department name, it means mapping failed
//       if (departmentId === departmentName && !departmentNameToIdMap.has(departmentName)) {
//         console.warn(`Department "${departmentName}" not found in mapping. Available departments:`, Array.from(departmentNameToIdMap.keys()));
//       }

//       console.log(`Fetching feedback for department: "${departmentName}" -> ID: "${departmentId}"`);

//       const response = await feedbackService.searchByDepartment(departmentId, 1, 100);

//       console.log(`API Response for ${departmentName}:`, response);

//       if (response.success) {
//         const feedbackData = Array.isArray(response.data) ? response.data : (response.data?.data || []);
//         console.log(`Found ${feedbackData.length} feedback messages for ${departmentName}`);
...
//         console.warn(`API returned error for ${departmentName}:`, response);
//         setFeedbackList([]);
...
//     }
//   }, [showError, departmentNameToIdMap]);
...
//                     </span>
//                     <button
//                       onClick={() => fetchDepartmentFeedback(dept.department)}
//                       className="ml-3 text-blue-600 hover:text-blue-800 text-sm font-medium flex items-center gap-1 inline-flex"
//                     >
...
//           <div key={selectedDepartmentForFeedback} id="feedback-messages-section" className="bg-white rounded-xl shadow-sm border border-gray-100">
//             <div className="p-6 border-b border-gray-100">
//               <div className="flex items-center justify-between">
//                 <h2 className="text-xl font-bold text-gray-900 flex items-center gap-3">
//                   <FiMessageSquare className="w-6 h-6 text-blue-600" />
//                   Feedback Messages - {selectedDepartmentForFeedback}
//                 </h2>
//                 <button
//                   onClick={() => {
//                     setFeedbackList([]);
//                     setSelectedDepartmentForFeedback('');
//                     setFeedbackLoading(false);
//                   }}
...
//                   <p className="text-gray-600">No feedback messages found for {selectedDepartmentForFeedback}.</p>
//                 </div>
//               )}
//             </div>
//           </div>
