...
export default function Recurring() {
  ...
  const fetchEvents = useCallback(async (page = 1) => {
    ...
    try {
      ...
      const response = await axios.get(`${BASE_URL}/events/recurring`, { params });
      ...
      setError(err.response?.data?.message || 'Failed to load recurring events');
      setEvents([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleEdit = async (eventId, field, value) => {
    ...
    await axios.put(`${BASE_URL}/events/recurring/${eventId}`, data);
    fetchEvents(pagination.currentPage);
  };
