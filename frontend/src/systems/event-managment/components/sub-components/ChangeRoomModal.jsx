...
export default function ChangeRoomModal({ isOpen, onClose, event, eventMode, onSuccess }) {
  ...
  const [format, setFormat] = useState(event.eventFormat || 'Physical');
  const [virtualLink, setVirtualLink] = useState(event.virtualLink || '');
  const [virtualDescription, setVirtualDescription] = useState(event.virtualDescription || '');
  const [linkError, setLinkError] = useState(null);
  const { showSuccess, showError } = useToast();

  const isCurrentlyVirtual = event.eventFormat === 'Virtual';
  ...
  const fetchRoomAvailability = useCallback(async () => {
    ...
    try {
      ...
      const currentRoomName = isCurrentlyVirtual ? '' : event.eventRoom?.toLowerCase();
  ...
  useEffect(() => {
    if (!isOpen || format === 'Virtual') return;
    fetchRoomAvailability();
  }, [isOpen, format, fetchRoomAvailability]);
  ...
  const handleSave = async () => {
    ...
    if (format === 'Virtual') {
      ...
      data = { eventFormat: 'Physical', eventRoom: selectedRoom };
    }
    ...
    try {
      ...
      if (res.data.success) {
        showSuccess(res.data.message || 'Location changed successfully');
        onSuccess?.(res.data.data);
        onClose();
      } else {
        showError(res.data.message || 'Failed to change location');
      }
    } catch (err) {
      showError(err.response?.data?.message || err.message);
    } finally {
  ...
  const canSave = format === 'Virtual' ? !saving : (!saving && !!selectedRoom && !loading);
        ...
        <div className="px-4 sm:px-6 py-5 space-y-4 max-h-[60vh] overflow-y-auto">
          <div className="p-3" style={{ backgroundColor: PRIMARY_TINT, border: `1px solid ${BORDER}` }}>
            <p className="text-xs" style={{ color: PRIMARY, fontFamily: fontHeading }}>
              Current location: <strong className="capitalize">{isCurrentlyVirtual ? 'Virtual' : event.eventRoom}</strong>. Choose a format and a new location below.
            </p>
          </div>
          ...
          {format !== 'Virtual' && !loading && rooms.available.length === 0 && rooms.unavailable.length === 0 && (
          ...
          {format !== 'Virtual' && !loading && rooms.available.length > 0 && (
            <div>
              ...
              <div className="space-y-2">
                {rooms.available.map((item, idx) => {
                  ...
                  const isCurrent = !isCurrentlyVirtual && item.room.roomName.toLowerCase() === event.eventRoom?.toLowerCase();
                  return (
                    <button key={idx} type="button"
                      onClick={() => setSelectedRoom(item.room.roomName)}
                      className={`w-full text-left p-3 border-2 ppp-lg transition-all duration-200 cursor-pointer ${
                        isSelected
          ...
          {format !== 'Virtual' && !loading && rooms.unavailable.length > 0 && (
            <div className="mt-3">
