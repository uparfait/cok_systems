  ...
  return {
    ...
    default_config: {
      ...
      rules: [
        ...
        "mode 'constant': 'value' is the stored answer (string for a select/cascading/text field, number for number/likert, an array of option values for multi_select/ranking, a stored location NAME for an API-sourced location field - the name exactly as the location API returns it, e.g. the province 'Umujyi wa Kigali' (its raw name, not the translation 'Kigali City'), the district 'Gasabo'; the builder offers these as a fetched dropdown so they are never typed).",
        ...
        "A preset parent plus by_parent children is how a whole cascade is fixed: Province constant 'Umujyi wa Kigali', District by_parent { 'Umujyi wa Kigali': 'Gasabo' }, and so on - by_parent keys are the parent's STORED answers (a location's raw name, an option's value).",
      ],
      example_constant: { id: "cascading_select_prov01", type: "cascading_select", label: Object.assign({}, TRANSLATED_TEXT_EXAMPLE, { en: "Province" }), data_source: { type: "api", level: "provinces" }, mandatory: false, validation_rules: [], default_config: { enabled: true, mode: "constant", value: "Umujyi wa Kigali" }, design: { spacing_below_px: 16 } },
      example_by_parent: { id: "cascading_select_dist01", type: "cascading_select", label: Object.assign({}, TRANSLATED_TEXT_EXAMPLE, { en: "District" }), parent_field_id: "cascading_select_prov01", data_source: { type: "api", level: "districts" }, mandatory: false, validation_rules: [], default_config: { enabled: true, mode: "by_parent", parent_field_id: "cascading_select_prov01", by_parent: { "Umujyi wa Kigali": "Gasabo", Amajyaruguru: "Musanze" } }, design: { spacing_below_px: 16 } },
    },
