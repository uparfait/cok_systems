...
/** One found record: its dates as the title, a few answers, its history on demand, and Load. */
function RecordCard({ record, fields, tracking, isLoaded, onLoad }) {
  ...
  return (
    <div className="border p-3" style={{ borderColor: isLoaded ? PRIMARY : BORDER, backgroundColor: isLoaded ? "rgba(5,109,170,0.05)" : "#FFFFFF" }}>
      ...
      <div className="flex flex-col min-[480px]:flex-row gap-2 mt-3">
        <DcsButtonOutline className="flex-1" onClick={() => setShowHistory(!show_history)}>
          {translate(show_history ? "DCS_TRACKING_HIDE_HISTORY" : "DCS_TRACKING_SHOW_HISTORY")}
        </DcsButtonOutline>
 ...
 * date it was recorded, with its answers, its whole change history and a
 * Load action that fills the form with it. When a record is already
 * loaded, its own history and each updatable field's value timeline open
 * first, with the search underneath to pick another.
 *
 * Full screen on a phone, a centered card from tablet width up.
 */
        ...
        <div className="flex-1 min-h-0 overflow-y-auto px-4 pt-4 pb-4 space-y-4">
          {loadedRecord && (
            <div className="border-2 p-3 space-y-3" style={{ borderColor: PRIMARY }}>
              ...
              <RecordHistoryList record={loadedRecord} fields={fields} />
              <FieldTimeline record={loadedRecord} fields={fields} editableIds={tracking.editable_field_ids} />
            </div>
          )}
