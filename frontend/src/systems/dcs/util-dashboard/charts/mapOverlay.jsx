 ...
 *
 * A place is only given an element when its own boundary is big enough on
 * screen to hold what it would say - so nothing is ever written across a
 * neighbour, and zooming in is what reveals the rest.
 */
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      places.forEach((place) => {
        ...
        if (Math.abs(far.x - near.x) < place.needed[0] || Math.abs(far.y - near.y) < place.needed[1]) return;
        keep.push(place);
      });
