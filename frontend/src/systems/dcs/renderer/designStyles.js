...
export function build_design_styles(field) {
  ...
  else if (design.background_color) inner_style.borderRadius = TINTED_BLOCK_RADIUS;
  if (inner_style.backgroundColor || inner_style.border) inner_style.padding = "0.75rem";
  return { outer_style, inner_style };
}

/**
 * A text block given a background is drawn as a soft, borderless tint with
 * rounded corners - the color alone marks it out, so it never gets a frame
 * unless the author switches the border on themselves.
 */
const TINTED_BLOCK_RADIUS = "8px";
