 ...
 * longer exists anywhere in the cloned copy.
 */
function build_id_remap(fields, accumulator) {
  const id_map = accumulator || new Map();
  (fields || []).forEach((field) => {
    if (!field || !field.id) return;
    id_map.set(field.id, generate_id(field.type));
 ...
 * never call clone_resolved_field directly from outside this module, or
 * cross-references silently break (the exact bug this fixes).
 */
function clone_resolved_fields(fields, template_id) {
