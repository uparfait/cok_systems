...
const allowe_resources = require('../../resources/resources.js')
...
class SystemPermissionsController {
    ...
    static async create(req, res, next) {
        try {
            ...
            const resourceNames = allowe_resources.map(r => r.resource_name.toLowerCase())
            if (!resourceNames.includes(resource.trim().toLowerCase())) {
                return res.status(400).json({
                    success: false,
                    type: "warning",
                    message: "Resource is not in the allowed list of resources"
                })
            }
    ...
    static async update(req, res, next) {
        try {
            ...
            // check if resource is provided and is a string and is in alowed list
            ...
            if (resource !== undefined) {
                const resourceNames = allowed_resources.map(r => r.resource_name.toLowerCase())
                if (!resourceNames.includes(resource.trim().toLowerCase())) {
                    return res.status(400).json({
                        success: false,
                        type: "warning",
