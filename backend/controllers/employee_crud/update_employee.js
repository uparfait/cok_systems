  ...
  try {
    ...
    if (newDeptId && newDeptId !== "Not specified" && newDeptId !== oldDeptId) {
      ...
      if (oldDeptId && oldDeptId !== "Not specified") {
        ...
        if (oldDept) {
          oldDept.total_employees = Math.max(
            0,
            (oldDept.total_employees || 0) - 1,
          );
          await oldDept.save();
        }
      }
    ...
    // also update department unit if specified and different from current
    if (
      newDeptUnitId &&
      newDeptUnitId !== "Not specified" &&
      newDeptUnitId !== oldDeptUnitId
    ) {
      if (!mongoose.Types.ObjectId.isValid(newDeptUnitId)) {
        return res.status(400).json({
          success: false,
          type: "warning",
          message: "Invalid department unit ID format.",
        });
      }
      const newDeptUnit = await department_model.findById(newDeptUnitId);
      if (!newDeptUnit) {
        return res.status(400).json({
          success: false,
          type: "warning",
          message: "Department unit not found.",
        });
      }
      // Decrement old department unit employee count
      if (oldDeptUnitId && oldDeptUnitId !== "Not specified") {
        const oldDeptUnit = await department_model.findById(oldDeptUnitId);
        if (oldDeptUnit) {
          oldDeptUnit.total_employees = Math.max(
            0,
            (oldDeptUnit.total_employees || 0) - 1,
          );
          await oldDeptUnit.save();
        }
      }

      // Increment new department unit using total_employees
      newDeptUnit.total_employees = (newDeptUnit.total_employees || 0) + 1;
      await newDeptUnit.save();
    } else if (
      newDeptUnitId === "Not specified" &&
      user.department_unit &&
      user.department_unit !== "Not specified"
    ) {
      // If changing to 'Not specified', decrement old department unit using total_employees
      const oldDeptUnit = await department_model.findById(user.department_unit);
      if (oldDeptUnit) {
        oldDeptUnit.total_employees = Math.max(
          0,
          (oldDeptUnit.total_employees || 0) - 1,
    ...
    if (department_id)
      user.department =
        department_id === "Not specified" ? null : department_id;
    // Save the department unit safely
    if (department_unit !== undefined)
      user.department_unit =
        department_unit === "Not specified" ? null : department_unit;

    if (identification) {
      if (identification.id_type !== undefined)
        user.identification.id_type = identification.id_type;
      if (identification.number !== undefined)
        user.identification.number = identification.number;
    }

    if (access_control) {
      if (access_control.is_locked !== undefined)
        user.access_control.is_locked = access_control.is_locked;
      if (access_control.reason !== undefined)
        user.access_control.reason = access_control.reason;
    }
      ...
      if (roles.permissions !== undefined) {
        if (Array.isArray(roles.permissions)) {
          ...
          for (const perm of roles.permissions) {
            const resourceName =
              perm.resource?.trim() || perm.resource_name?.trim();
            ...
            const resourceDef = allowed_resources.find(
              (r) =>
                r.resource_name.toLowerCase() === resourceName.toLowerCase(),
            );
            ...
            actionsToEnable.forEach((action) => {
              const actionType =
                typeof action === "string" ? action : action.action_type;
              const validActionTypes = resourceDef.actions.map(
                (a) => a.action_type,
              );
