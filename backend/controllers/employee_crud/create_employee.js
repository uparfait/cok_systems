...
module.exports = async function create_user(req, res, next) {
  try {
    ...
    if (!full_name || !email || !telephone) {
      await logAuditEvent(
        "SYSTEM",
        "Employee creation failed: Missing required fields",
        req,
        {
          resource: "users",
          status_code: 400,
    ...
    if (department_id && department_id !== "Not specified") {
      if (mongoose.Types.ObjectId.isValid(department_id)) {
        ...
        if (!dept) {
          await logAuditEvent(
            "SYSTEM",
            `Employee creation failed: Department not found (${department_id})`,
            req,
            {
        ...
        await logAuditEvent(
          "SYSTEM",
          `Employee creation failed: Invalid department ID format (${department_id})`,
          req,
          {
            resource: "departments",
            status_code: 400,
    ...
    if (department_unit && department_unit !== "Not specified") {
      if (mongoose.Types.ObjectId.isValid(department_unit)) {
        ...
        if (!deptUnitExists) {
          await logAuditEvent(
            "SYSTEM",
            `Employee creation failed: Department unit not found (${department_unit})`,
            req,
            {
              resource: "departments",
        ...
        await logAuditEvent(
          "SYSTEM",
          `Employee creation failed: Invalid department unit format (${department_unit})`,
          req,
          {
            resource: "departments",
            status_code: 400,
    ...
    if (query_conditions.length > 0) {
      ...
      if (existing_user) {
        ...
        await logAuditEvent(
          "SYSTEM",
          `Employee creation failed: ${conflict_field} already exists (${email || telephone})`,
          req,
          {
            resource: "users",
            status_code: 409,
    ...
    if (roles.permissions && Array.isArray(roles.permissions)) {
      for (const perm of roles.permissions) {
        ...
        if (!resourceDef) {
          await logAuditEvent(
            "SYSTEM",
            `Employee creation failed: Invalid resource (${resourceName})`,
            req,
            {
    ...
    await logAuditEvent(
      "CREATE",
      `Employee account created: ${full_name} (${email})`,
      req,
      {
        resource: "users",
    ...
    if (
      ...
      if (dpt_to_update && dpt_to_update._id.toString() !== department_id) {
        dpt_to_update.total_employees =
          (dpt_to_update.total_employees || 0) + 1;
    ...
    return res.status(201).json({
      ...
      type: "success",
      message:
        "Employee account created successfully. Account activation is required.",
    });
  } catch (error) {
    // Log system error
    await logAuditEvent(
      "ERROR",
      `Employee creation system error: ${error.message}`,
      req,
      {
        resource: "users",
        status_code: 500,
